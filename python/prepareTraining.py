...
def prepareTraining(output, sigList=None, bkgList=None, preselection=None, nvar=None, weight=None, lumi=100e3, trainSize=None, testSize=None, reproduce=False, multiclass=False):
  ...
  if not os.path.exists(output):
    ...
    sig = np.empty([0, Signal[0].dataframe[0].shape[1]], dtype=np.float64)
    sig_w = np.empty(0, dtype=np.float64)
    sig_y = np.empty(0, dtype=np.float64)
    bkg = np.empty([0, Background[0].dataframe[0].shape[1]], dtype=np.float64)
    bkg_w = np.empty(0, dtype=np.float64)
    bkg_y = np.empty(0, dtype=np.float64)
    ...
    h5f.create_dataset('X', data=X.astype('float64'))
    h5f.create_dataset('y', data=y)
    h5f.create_dataset('w', data=w.astype('float64'))
    h5f.create_dataset('X_train', data=X_train.astype('float64'))
    h5f.create_dataset('X_test', data=X_test.astype('float64'))
    h5f.create_dataset('y_train', data=y_train)
    h5f.create_dataset('y_test', data=y_test)
    h5f.create_dataset('w_train', data=w_train.astype('float64'))
    h5f.create_dataset('w_test', data=w_test.astype('float64'))
