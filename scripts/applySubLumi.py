...
def main():
  ...
  for f in inFiles:
    ...
    for name in set([k.GetName() for k in f.GetListOfKeys() if k.GetClassName() == "TTree"]):
      ...
      sub_campaign = original_tree[original_tree.find("mc16") + 4]
      if sub_campaign == "a":
      ...
      friendTree = ROOT.TTree(friendName, friendName)

      lumi_branch = friendTree.Branch("weight_lumi", LUMI_ARRAY, "weight_lumi/F")
