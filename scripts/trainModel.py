...
def main():
  ...
  for ana in analysis:
    ...
    if (ana.name.lower() == 'nn'):
      model_DNN, y_pred_DNN = trainNN(X_train, X_test, y_train, y_test, w_train, w_test, ana.options['layers'], ana.options['ncycles'], ana.options['batchSize'], ana.options['dropout'], ana.options['optimizer'], ana.options['learningRate'], ana.options['decay'], ana.options['momentum'], ana.options['nesterov'], ana.options['multiclassification'])
